...
@WorkflowComponent
class Sd_Cfg_H extends Cfg_H_Template<Sd> {

   public static val EXTENSION_CFG_H = new Sd_Cfg_H
 ...
 public static SdGeneral Sd_gen
   SoAd _Soad = null
   Sd _Sd
   ...
   var static int config_opt_total_length = 0
    var public static int size_Array=0;
     var public static int Array_in = 0
       var public static int Array_in2 = 0
       var public static int Array_in3 = 0
       var public static int Array_in4 = 0
   
   
   var static long Sd_Acl_client_cnt = 0
   var static long Sd_Acl_Server_cnt = 0
   ...
   var static Integer counterval = 0
...
«IF isPreCompile(module).equals(true) »
#define SD_DBTOC_VALUE                   0       
«ELSE»
#define SD_DBTOC_VALUE             «dbtoc.toString»  
«ENDIF»
...
«IF Sd_General?.sdEnableSecurityEventReporting.booleanValue == true »
 ...
 «ENDIF»
    «IF stdOn(isPreCompile(module)) =="STD_ON"»
     ...
     «IF module?.sdConfig?.sdAclCheckBlockDescriptorRef?.nvMNvramBlockIdentifier !== null»
     #define SD_NVMBLOCK_ID                 («module.getNvMBlockId»)
     «ELSE»
     #define SD_NVMBLOCK_ID                  NULL_PTR
     «ENDIF»
     
      «IF module?.sdConfig?.sdAclCheckBlockDescriptorRef !== null»
      «IF (module.allserviceMaxConfigured==true)»
      #define SD_NO_OF_RAM_ALLOWED_SERVER_CLIENT_PROVIDER_CONSUMER      («module.get_number9+module.get_number10»U)
      #define SD_RAM_BLOCK          STD_ON
      
      «ELSE»
     
      #define SD_NO_OF_RAM_ALLOWED_SERVER_CLIENT_PROVIDER_CONSUMER     («module.get_number11»U)
      #define SD_RAM_BLOCK          STD_ON
...
#define SD_RAM_BLOCK          STD_OFF
 ...
 «ENDIF»
    «IF (module?.sdConfig?.sdAclCheckBlockDescriptorRef !== null) && (module?.sdConfig?.sdAclCheckBlockDescriptorRef?.nvMNvBlockLength !== null)»
     #define SD_NVM_BLOCK_LENGTH («module?.sdConfig?.sdAclCheckBlockDescriptorRef?.nvMNvBlockLength.intValue»U)
    «ELSE»
     #define SD_NVM_BLOCK_LENGTH («0»U)
    «ENDIF»
     «IF SdInstance.map[sdClientServices.map[sdClientServiceAllowedProviders_PB]].flatten.flatten.size+SdInstance.map[sdServerServices.map[sdServerServiceAllowedConsumerss_PB]].flatten.flatten.size > 0»
       ...
       #define SD_NO_OF_ROM_ALLOWED_SERVER_CLIENT_PROVIDER_CONSUMER     («SdInstance.map[sdClientServices.map[sdClientServiceAllowedProviders_PB]].flatten.flatten.size+SdInstance.map[sdServerServices.map[sdServerServiceAllowedConsumerss_PB]].flatten.flatten.size»U)
       
       #define SD_ROM_BLOCK          STD_ON
...
typedef struct STag_Sd_ProviderConsumerList
{
    ...
    typedef struct STag_Sd_NvMRamBlockData{
    	
    unsigned int bl_Sd_ACL_PolicyCheck:1;
    unsigned int bl_Sd_Nvm_Read_Write_Block_Flag:1;
	Sd_ProviderConsumerListType Sd_GaaServerClientList[SD_NO_OF_RAM_ALLOWED_SERVER_CLIENT_PROVIDER_CONSUMER];
    }Sd_NvMRamBlockData_type;
#endif	
	
#if(SD_ROM_BLOCK == STD_ON)
    typedef struct STag_Sd_NvMRomBlockData{
    	
     unsigned int bl_Sd_ACL_PolicyCheck:1;
     unsigned int bl_Sd_Nvm_Read_Write_Block_Flag:1;
	 Sd_ProviderConsumerListType Sd_GaaServerClientList[SD_NO_OF_ROM_ALLOWED_SERVER_CLIENT_PROVIDER_CONSUMER];
    	
    
		
		
	}Sd_NvMRomBlockData_type;
#endif	
«ELSE»
typedef enum ETag_Sd_IpAddressType
{
...
«ENDIF»
    ...
    if(client_cap_rec_list.size > 0)
    {
      client_service_hash.put(client_service_path, client_cap_rec_list)
        ...
        for(cl_servTcpRef : cl_service?.sdClientServiceTcpRef_PB?.toList)
        {
          ...
          for(sokcon : _Soad?.soAdConfig?.soAdSocketConnectionGroups)
          {
     ...
     if(con_eve_cap_rec_list.size > 0)
     {
      ...
      con_ev_grp_ref = new ArrayList<String>()
        ...
        if(multigrp?.reference?.shortNamePath?.toString != null)
        {
                ...
                for(sokcongrp : _Soad?.soAdConfig?.soAdSocketConnectionGroups)
                {
   ...
   def get_tcpip_domain_type()
   {
     ...
     [
       ...
       [
         ...
         for(soad_con_grps: _Soad?.soAdConfig?.soAdSocketConnectionGroups)
         {
  ...
  def get_config_opt_consumed_eve()
  {
    ...
    def get_server_arry1()
   {
      ...
      /**
   * Generates type of maximum value of timeout related parameters
   * for embedded code.
   * @param : None
   * @return : String
   */  
  ...
  else if (HandleID == 4)
  {
    ...
    def getNvMBlockId(Sd module) {
        var getid = 0
        if(module?.sdConfig?.sdAclCheckBlockDescriptorRef !== null)
        {
               getid = module?.sdConfig?.sdAclCheckBlockDescriptorRef ?.nvMNvramBlockIdentifier
                return getid
        }
        else{
         ...
         def get_number4()
   {
   	...
   	return Array_in4
          ...
          def get_number5(Sd module)
   {
 Sd_Acl_client_cnt = 0
  for(sd_inst : module?.sdConfig?.sdInstances){
   for (client_service : sd_inst?.sdClientServices){
     Sd_Acl_client_cnt  +=   	client_service ?.sdMaxNumOfIpAddressesInAcl?.intValue
   	
   }
   
   }
   
   if(Sd_Acl_client_cnt == 0){
   	 for(sd_inst : module?.sdConfig?.sdInstances){
   for (client_service : sd_inst?.sdClientServices){
     Sd_Acl_client_cnt  +=   	client_service ?.sdClientServiceAllowedProviders_PB.size
   	...
   	return Sd_Acl_client_cnt
   	
   }
      def get_number6(Sd module)
   {
   	Sd_Acl_Server_cnt = 0
 
  for(sd_inst : module?.sdConfig?.sdInstances){
   for (client_service : sd_inst?.sdServerServices){
     Sd_Acl_Server_cnt  +=   	client_service?.sdMaxNumOfIpAddressesInAcl?.intValue
   	
   }

   }
   if(Sd_Acl_Server_cnt == 0)
   {
   	 for(sd_inst : module?.sdConfig?.sdInstances){
   for (client_service : sd_inst?.sdServerServices){
     Sd_Acl_Server_cnt  +=   	client_service ?.sdServerServiceAllowedConsumerss_PB?.size
   	
   }
   }
  
   	return Sd_Acl_Server_cnt
   	
   }
   
   }
   
    def get_number7(Sd module)
   {
   	Sd_Acl_Server_cnt = 0
  ...
  for(sd_inst : module?.sdConfig?.sdInstances){
   for (client_service : sd_inst?.sdServerServices){
     ...
     def get_number8(Sd module)
   {
   	Sd_Acl_Server_cnt = 0
 
  for(sd_inst : module?.sdConfig?.sdInstances){
   for (client_service : sd_inst?.sdClientServices){
     Sd_Acl_Server_cnt  +=   	client_service ?.sdClientServiceAllowedProviders_PB?.size
   ...
   def allserviceMaxConfigured(Sd module)
   {
   ...
   if(flag==true)
   {
   	for(sd_inst1 : module?.sdConfig?.sdInstances){
   for (client_service : sd_inst1?.sdClientServices){
     Sd_Acl_client_cnt  =   	client_service ?.sdMaxNumOfIpAddressesInAcl?.intValue
   	if(Sd_Acl_client_cnt==0)
     {
     	flag = false
   ...
   for (client_service : sd_inst?.sdClientServices){
     Sd_Acl_client_cnt  +=   	client_service ?.sdMaxNumOfIpAddressesInAcl?.intValue
   	...
   	return Sd_Acl_client_cnt
             ...
             def get_number11(Sd module)
   {
 Sd_Acl_client_cnt = 0
 if(module.sdConfig.sdAclCheckBlockDescriptorRef !== null){
Sd_Acl_client_cnt =(module.sdConfig.sdAclCheckBlockDescriptorRef?.nvMNvBlockLength -4)/32
  ...
  def get_number12(Sd module)
   {
 Sd_Acl_client_cnt = 0
  for(sd_inst : module?.sdConfig?.sdInstances){
   for (client_service : sd_inst?.sdClientServices){
   	if(client_service?.sdMaxNumOfIpAddressesInAcl?.intValue > client_service?.sdClientServiceAllowedProviders_PB?.size)
    { Sd_Acl_client_cnt  +=   	(client_service )?.sdMaxNumOfIpAddressesInAcl?.intValue}
   ...
   for (client_service : sd_inst?.sdClientServices){
     Sd_Acl_client_cnt  +=   	client_service ?.sdClientServiceAllowedProviders_PB.size
   	...
   	return Sd_Acl_client_cnt
         ...
         def get_number13(Sd module)
   {
   	Sd_Acl_Server_cnt = 0
 
  for(sd_inst : module?.sdConfig?.sdInstances){
   for (client_service : sd_inst?.sdServerServices){
   	if(	client_service ?.sdMaxNumOfIpAddressesInAcl?.intValue > 	client_service?.sdServerServiceAllowedConsumerss_PB.size)
     {Sd_Acl_Server_cnt  +=   	(client_service )?.sdMaxNumOfIpAddressesInAcl?.intValue}
