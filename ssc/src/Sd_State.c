...
/* parasoft-begin-suppress CERT_C-POS54-a-1 "Reason: Parasoft warnning is about
 * value returned by the POSIX library function 'memset' should be used" 
 * "Justification: Even though the Function is having Void return type parasoft 
 * is asking to use its return type" */

 /* parasoft-begin-suppress MISRAC2012-RULE_17_7-a-2 "Reason: Parasoft warnning 
 * is about value returned by the POSIX library function 'memset' should be used" 
 * "Justification: Even though the Function is having Void return type parasoft 
 * is asking to use its return type" */

 /* parasoft-begin-suppress CERT_C-EXP12-a-3 "Reason: Parasoft warnning is about
 * value returned by the POSIX library function 'memset' should be used" 
 * "Justification: Even though the Function is having Void return type parasoft 
 * is asking to use its return type" */
...
/* #section Sd_State_c_Cert_REF_3
* Violates CERT_C-POS49-a-2: "Reason: warning is about ensuring atomic access 
* to bit fields, especially when multiple bit fields are in the same structure. 
* Justification: Variable is accessed in critical section only so there should 
* not be a issue." */

/* #section Sd_State_c_Cert_REF_4
* Violates CERT_C-CON32-a-2: "Reason: warning is about ensuring atomic access 
* to bit fields, especially when multiple bit fields are in the same structure. 
* Justification: Variable is accessed in critical section only so there should 
* not be a issue." */
...
#include "Sd_MemMap.h"
static void Sd_ServerServiceWaitEntered(
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_ServerStateInitialWait(
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_ServerStateRepetition(
    Sd_ServerServiceType *const LpServerService,
    const Sd_ServerServiceStaticType *const LpServerServiceStatic);
#define SD_STOP_SEC_CODE
#include "Sd_MemMap.h"
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_ServerStateMain(
    Sd_ServerServiceType *const LpServerService,
    const Sd_ServerServiceStaticType *const LpServerServiceStatic);
#define SD_STOP_SEC_CODE
#include "Sd_MemMap.h"
...
static void Sd_ResetToWildcardServer(
    const Sd_InstanceStaticType *const LpInstanceStatic,
    const Sd_EvHandlerSubGrpType *const LpEvHandlerSubGrp,
...
#include "Sd_MemMap.h"
/* polyspace-begin RTE:UNR [Justified:Low] "Refer Sd_c_Poly_REF_1"*/
static Std_ReturnType Sd_AddClientToFanoutTcp(
    const Sd_ServerServiceStaticType *LpServerServiceStatic,
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_ClientStateInitialWait(
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_ClientStateRepetition(
    Sd_ClientServiceType *LpClientService,
    const Sd_ClientServiceStaticType *LpClientServiceStatic);
#define SD_STOP_SEC_CODE
#include "Sd_MemMap.h"
...
#include "Sd_MemMap.h"
/* polyspace-begin RTE:UNR [Justified:Low] "Refer Sd_c_Poly_REF_1"*/
static void Sd_ClientStateMain(
    Sd_ClientServiceType *LpClientService,
    const Sd_ClientServiceStaticType *LpClientServiceStatic,
...
#include "Sd_MemMap.h"
/* polyspace-begin RTE:UNR [Justified:Low] "Refer Sd_c_Poly_REF_1"*/
static Std_ReturnType Sd_ClientSetUpTcpConnection(
...
#include "Sd_MemMap.h"
/* polyspace-begin RTE:UNR [Justified:Low] "Refer Sd_c_Poly_REF_1"*/
static Std_ReturnType Sd_ClientSetUpUdpConnection(
    const Sd_InstanceStaticType *LpInstanceStatic,
    Sd_ClientServiceType *LpClientService,
    const Sd_ClientServiceStaticType *LpClientServiceStatic,
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static boolean Sd_CheckSoConOnline(uint16 LusBaseIndex,
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_OpenSoConsAndEnableRouting(uint16 LusBaseIndex, 
  uint16 LusNoOfElements, 
  const Sd_ClientServiceStaticType *LpClientServiceStatic);
#define SD_STOP_SEC_CODE
#include "Sd_MemMap.h"
    ...
    const Sd_ServerServiceStaticType *const LpServerServiceStatic)
{
  ...
  /* Single exit at end of function */
}
#define SD_STOP_SEC_CODE
#include "Sd_MemMap.h"
#endif
...
"Reason: Sd_State_c_Cert_REF_1" */
/* parasoft-begin-suppress CERT_C-DCL00-b-3 
  "Reason: Sd_State_c_Cert_REF_2" */
static void Sd_ServerServiceWaitEntered(
    Sd_ServerServiceType * const LpServerService,
    const Sd_ServerServiceStaticType *const LpServerServiceStatic)
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if (SD_TRUE == (LpServerServiceStatic->stServerStaticFlag).blActivationRoGrpIdPresent)
  {
    (void)SoAd_DisableRouting(LpServerServiceStatic->ddActivationRoGrpId);
  }
  LpTimer = LpServerServiceStatic->pServerTimer;
  LpServerService->ulServerDelayCount = Sd_Rand((uint32)LpTimer->ulInitialOfferDelayMin,
...
{
#if (STD_ON == SD_EV_HANDLER_CONFIGURED)
  Sd_EvHandlerType *LpEvHandler = SD_NULL_PTR;
  Sd_EvHandlerStaticType const *LpEvHandlerStatic = SD_NULL_PTR;
  uint16 LusBaseIndex = SD_ZERO;
  uint16 LusCount = SD_ZERO;
  uint16 LusNoOfElements = SD_ZERO;
#endif
  Sd_ServerTimerStaticType const *LpServerTimer = SD_NULL_PTR;
  ...
  if (SD_SERVER_SERVICE_AVAILABLE != LpServerService->enServerServiceSetState)
  {
    ...
    for (LusCount = LusBaseIndex;
         LusCount < (uint16)(LusBaseIndex + LusNoOfElements); LusCount++)
    {
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  Sd_EvHandlerStaticType const *LpEvHandlerStatic = SD_NULL_PTR;
#endif
  uint16 LusMultiple = SD_ZERO;
  uint16 LusCount = SD_ZERO;
  SoAd_SoConIdType LddSoConId = (SoAd_SoConIdType) SD_ZERO;
  SoAd_SoConModeType LenSoConMode = SOAD_SOCON_OFFLINE;
  ...
  if (LpServerServiceStatic->pTcpSoConGrp != SD_NULL_PTR)
  {
    LusBaseIndex = LpServerServiceStatic->pTcpSoConGrp->usBaseSocketConIndex;
    LusNoOfElements = LpServerServiceStatic->pTcpSoConGrp->usNoOfSockets;
    for (LusCount = LusBaseIndex; (LusCount < (uint16)(LusBaseIndex + LusNoOfElements)) && (!LblSoConOnline); LusCount++)
    {
  ...
  if (LblSoConOnline)
  {
    if (SD_SERVER_SERVICE_AVAILABLE != LpServerService->enServerServiceSetState)
    {
      ...
      LusBaseIndex = LpServerServiceStatic->usBaseEvHandlerIndex;
      LusNoOfElements = LpServerServiceStatic->usNoOfEvHandlers;
      for (LusCount = LusBaseIndex; LusCount < (uint16)(LusBaseIndex + LusNoOfElements); LusCount++)
      {
        LpEvHandler = &Sd_GaaEvHandler[LusCount];
        #if (STD_ON == SD_PRE_COMPILE_SINGLE)
        LpEvHandlerStatic = &Sd_GaaEvHandlerStatic[LusCount];
    ...
    { /* SWS_SD_00329
      ...
      if (LpServerService->ulServerDelayCount == (uint32)SD_ZERO)
      {
        ...
        if (LpServerService->ucNoOfOffersSent < LpServerTimer->ucInitialOfferRepetitionsMax)
        {
          LusMultiple = SD_ONE;
          /*  SWS_SD_00331
              SWS_SD_00363a
          ...
          for (LusCount = SD_ZERO; LusCount < 
            (uint16)(LpServerService->ucNoOfOffersSent); LusCount++)
          {
            LusMultiple = LusMultiple * SD_TWO;
          }
          /* parasoft-begin-suppress CERT_C-INT02-b-2 "Reason: Parasoft thinking of 
          * datatypes mismatch happened." "Justification: The cast is necessary to
          * ensure the correct type is used." */
          /* parasoft-begin-suppress CERT_C-EXP14-a-3 "Reason: Parasoft thinking of 
          * datatypes mismatch happened." "Justification: The cast is necessary to
          * ensure the correct type is used." */
         /* parasoft-begin-suppress MISRAC2012-RULE_10_8-a-2 "Reason: Parasoft thinking of 
          * datatypes mismatch happened." "Justification: The cast is necessary to
          * ensure the correct type is used." */
          LpServerService->ulServerDelayCount = (uint32)((uint16)LusMultiple * 
            (uint16)(LpServerTimer->ulInitialOfferRepetitionBaseDelay));
          /* parasoft-end-suppress CERT_C-INT02-b-2 */
          /* parasoft-end-suppress CERT_C-EXP14-a-3 */
          /* parasoft-end-suppress MISRAC2012-RULE_10_8-a-2 */
        }
        /*  SWS_SD_00336  */
        else
        {
          /*  SWS_SD_00450  */
          LpServerService->ulServerDelayCount = 
          (uint32)(LpServerTimer->ulTimerOfferCyclicDelay);
          LpServerService->enServerPhase = SD_SERVER_MAIN;
        }
      }
    }
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  else
  {
    ...
    if ((LpServerTimer->ulTimerOfferCyclicDelay > (uint32)SD_ZERO) && ((uint32)SD_ZERO == (uint32)LpServerService->ulServerDelayCount))
    {
      LpServerService->ulServerDelayCount = LpServerTimer->ulTimerOfferCyclicDelay;
      Sd_SendOffer(LpServerServiceStatic, SD_NULL_PTR, SD_ZERO, (boolean)SD_FALSE);
    }
  }
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  uint8 LucIsValidPtr = SD_FALSE;
  /* Pointer validation for all pointer parameters */
  if ((SD_NULL_PTR != LpInstance) &&
      (SD_NULL_PTR != LpInstanceStatic) &&
      (SD_NULL_PTR != LpServerService) &&
      (SD_NULL_PTR != LpServerServiceStatic))
  {
    LucIsValidPtr = SD_TRUE;
  }

  if (SD_TRUE == LucIsValidPtr)
  {
    ...
    case SD_SERVER_DOWN:
    {
      ...
      /*  SWS_SD_00606b  */
      if ((Sd_GblInitCalled) &&
          (SD_SERVER_SERVICE_AVAILABLE == LpServerService->enServerServiceSetState) &&
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  /* Pointer validation for all pointer parameters */
  if ((SD_NULL_PTR != LpServerService) &&
      (SD_NULL_PTR != LpEvHandler) &&
      (SD_NULL_PTR != LpEvHandlerStatic))
  {
    LucIsValidPtr = SD_TRUE;
  }
  ...
  if (SD_TRUE == LucIsValidPtr)
  {
    ...
    for (LusCount = LusBaseIndex; LusCount < (uint16)(LusBaseIndex + SD_SIXTEEN);
         LusCount++)
    {
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if (LpSoConStructure->usNoOfSubServers > SD_ZERO)
  {
    ...
    {
    /* parasoft-begin-suppress CERT_C-POS49-a-2
      "Reason: Sd_State_c_Cert_REF_3" */
    /* parasoft-begin-suppress CERT_C-CON32-a-2
      "Reason: Sd_State_c_Cert_REF_4" */
      LpSoConStructure->stSoConFlag.blWildcardSocketServer = SD_FALSE;
    /* parasoft-end-suppress CERT_C-POS49-a-2 */
    /* parasoft-end-suppress CERT_C-CON32-a-2 */
      SchM_Exit_Sd_MODE_STATUS_PROTECTION();
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if (SD_TRUE == LucIsValidPtr)
  {
    ...
    for (LusCount = LusBaseIndex; LusCount < 
      ((uint16)(LusBaseIndex + SD_SIXTEEN)); LusCount++)
    {
      LpEvHandlerSubGrp = &Sd_GaaEvHandlerSubGrp[LusCount];
      ...
      if (LpEvHandlerSubGrp->ddSoConTcpIndex < SD_NO_OF_SOCKET_CONNECTIONS)
      {
        ...
        if (SD_TRUE == (LpEvHandlerSubGrp->stEvHandlerFlag).blTcpActiveEv)
        {
          ...
          if ((SD_TRUE == LpEvHandlerStatic->stEvHandlerStaticFlag.blTcpActivationRoGrpIdPresent))
          {
    ...
    if ((SD_ZERO == LpEvHandlerStatic->usMulticastThreshold) ||
        ((uint16)LpEvHandler->ucNoOfSubGrpsUdp < LpEvHandlerStatic->usMulticastThreshold))
    {
      LusBaseIndex = (uint16)LpEvHandlerStatic->usEvHandlerSelfIndex * SD_SIXTEEN;

      for (LusCount = LusBaseIndex; LusCount < (uint16)(LusBaseIndex + SD_SIXTEEN); LusCount++)
      {
        LpEvHandlerSubGrp = &Sd_GaaEvHandlerSubGrp[LusCount];
        ...
        if (LpEvHandlerSubGrp->ddSoConUdpIndex < SD_NO_OF_SOCKET_CONNECTIONS)
        {
          ...
          if (SD_TRUE == (LpEvHandlerSubGrp->stEvHandlerFlag).blUdpActiveEv)
          {
            ...
            if ((SD_TRUE == LpEvHandlerStatic->stEvHandlerStaticFlag.blUdpActivationRoGrpIdPresent))
            {
              (void)SoAd_DisableSpecificRouting(
    ...
    else
    {
      /* polyspace +4 RTE:UNR [Justified:Low] "This condition shall be True when
      blTcpActivationRoGrpIdPresent is not equal to False. */
      if ((SD_TRUE == (LpEvHandlerStatic->stEvHandlerStaticFlag).blMulticastActivationRoGrpIdPresent))
      {
        (void)SoAd_DisableSpecificRouting(
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if (SD_TRUE == LucIsValidPtr)
  {
    ...
    if ((SD_TCP_OPTIONS_MASK == (LucOptionsCode & SD_TCP_OPTIONS_MASK)) &&
        ...
        ((SD_TRUE == LpEvHandlerStatic->stEvHandlerStaticFlag.blTcpActivationRoGrpIdPresent) ||
         (SD_TRUE == LpEvHandlerStatic->stEvHandlerStaticFlag.blTcpTriggeringRoGrpIdPresent)))
    {
      ...
      for (LusCount = LusBaseIndex;
           (LusCount < (uint16)(LusBaseIndex + LusNoOfElements)) &&
           (!LblMatchRecd);
           LusCount++)
      {
      ...
      else if (LblWildcardMatch)
      {
        if (SOAD_SOCON_ONLINE == LenSoConMode)
        {
          ...
          Sd_GaaSoCon[LddWildcardSocketIndex].stSoConFlag.blWildcardSocketServer = SD_TRUE;
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_TCP_OPTIONS_MASK == (LucOptionsCode & SD_TCP_OPTIONS_MASK)) &&
      ...
      (SD_NULL_PTR != LpEvHandlerSubGrp) &&
      ((SD_TRUE == 
        LpEvHandlerStatic->stEvHandlerStaticFlag.blTcpActivationRoGrpIdPresent) ||
       (SD_TRUE == 
        LpEvHandlerStatic->stEvHandlerStaticFlag.blTcpTriggeringRoGrpIdPresent)))
  {
    ...
    if ((E_OK == LddReturnValue) && (LddConnectionReturnValue == (uint8)TCPIP_E_OK))
    {
      ...
      /* polyspace +4 RTE:UNR [Justified:Low] "This condition shall be True when
      LpEvHandlerStatic is SD_TRUE. */
      if ((SD_TRUE == LpEvHandlerStatic->stEvHandlerStaticFlag.blTcpTriggeringRoGrpIdPresent))
      {
        /*polyspace +10 RTE:OBAI [Justified:Low] "Pointers with this orange flag are
        made sure that they are within their range by having necessary boundary
        checks in order to prevent them from being out of bounds." */
...
/* parasoft-begin-suppress CERT_C-DCL00-b-3 
  "Reason: Sd_State_c_Cert_REF_2" */
static void Sd_AddClienttoFanoutUdpActions(
    Sd_EvHandlerType *LpEvHandler,
    const Sd_EvHandlerStaticType *LpEvHandlerStatic,
    Sd_EvHandlerSubGrpType *LpEvHandlerSubGrp,
    SoAd_SoConIdType LddMatchSocketIndex)
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  uint16 LusBaseIndex;
  uint16 LusCount;
  uint8 LucSkipLogic = SD_FALSE; /* For pointer validation */

  /* Parasoft CERT-C: Pointer parameter validation */
  if ((SD_NULL_PTR == LpEvHandler) ||
      (SD_NULL_PTR == LpEvHandlerStatic) ||
      (SD_NULL_PTR == LpEvHandlerSubGrp))
  {
    /* parasoft-suppress CERT_C-EXP34-a-1 "Pointer parameter validated for NULL as per ruleset. Logic skipped as per ruleset." */
    LucSkipLogic = SD_TRUE;
  }

  if (SD_FALSE == LucSkipLogic)
  {
    /* SWS_SD_00455
       SWS_SD_00455a
       SWS_SD_00455b
    ...
    /*polyspace +5 RTE:IDP [Justified:Low] "Pointers with this orange flag are
        ...
        checks in order to prevent them from being illegally dereferenced." */
    if ((SD_ZERO == LpEvHandlerStatic->usMulticastThreshold) ||
        ((uint16)LucNoOfSubGrpsUdp < LpEvHandlerStatic->usMulticastThreshold))
    {
      SchM_Enter_Sd_MODE_STATUS_PROTECTION();
    ...
    /* polyspace +8 RTE:UNR [Justified:Low] "This condition shall be True when
      LpEvHandlerStatic is SD_TRUE. */
    else if ((uint16)LucNoOfSubGrpsUdp == LpEvHandlerStatic->usMulticastThreshold)
    {
      LusBaseIndex = (uint16)LpEvHandlerStatic->usEvHandlerSelfIndex * SD_SIXTEEN;
      for (LusCount = LusBaseIndex; LusCount < (uint16)(LusBaseIndex +
                                                        SD_SIXTEEN);
  ...
  /* Single exit point as per ruleset */
}
/* polyspace-end RTE:UNR [Justified:Low] "Refer Sd_c_Poly_REF_1"*/
#define SD_STOP_SEC_CODE
#include "Sd_MemMap.h"
...
/* parasoft-begin-suppress CERT_C-DCL00-b-3 
  "Reason: Sd_State_c_Cert_REF_2" */
static Std_ReturnType Sd_FindUdpIndex(
    const Sd_ServerServiceStaticType *LpServerServiceStatic,
    const Sd_EvHandlerStaticType *LpEvHandlerStatic,
    Sd_EvHandlerSubGrpType *LpEvHandlerSubGrp,
    const TcpIp_SockAddrType *LpIpAddrUdp,
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if (SD_FALSE == LucSkipLogic)
  {
    ...
    if ((SD_UDP_OPTIONS_MASK == (LucOptionsCode & SD_UDP_OPTIONS_MASK)) &&
        ...
        ((SD_TRUE == LpEvHandlerStatic->stEvHandlerStaticFlag.blUdpActivationRoGrpIdPresent) ||
         (SD_TRUE == LpEvHandlerStatic->stEvHandlerStaticFlag.blUdpTriggeringRoGrpIdPresent)))
    {
      LblMatchRecd = (boolean)SD_FALSE;
      LblWildcardMatch = (boolean)SD_FALSE;
      LusBaseIndex = LpServerServiceStatic->pUdpSoConGrp->usBaseSocketConIndex;
...
/* parasoft-begin-suppress CERT_C-DCL00-b-3 
  "Reason: Sd_State_c_Cert_REF_2" */
static Std_ReturnType Sd_AddClientToFanoutUdp(
    const Sd_ServerServiceStaticType *LpServerServiceStatic,
    Sd_EvHandlerType *LpEvHandler,
    const Sd_EvHandlerStaticType *LpEvHandlerStatic,
    Sd_EvHandlerSubGrpType *LpEvHandlerSubGrp,
...
/* parasoft-begin-suppress CERT_C-DCL00-b-3 
  "Reason: Sd_State_c_Cert_REF_2" */
static Std_ReturnType Sd_AddClientCheck(
    const Sd_EvHandlerSubGrpType * const LpEvHandlerSubGrp,
    boolean LblTcp,
    const TcpIp_SockAddrType * const LpIpAddr)
/* parasoft-end-suppress CERT_C-API00-a-3 */
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  Std_ReturnType LddReturnValue;
  ...
  if ((SD_NULL_PTR != LpEvHandlerSubGrp) && (SD_NULL_PTR != LpIpAddr))
  {
    if (LblTcp)
    {
      if (LpEvHandlerSubGrp->ddSoConTcpIndex < SD_NO_OF_SOCKET_CONNECTIONS)
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_NULL_PTR == LpEvHandler) ||
      ...
      (SD_NULL_PTR == LpIpAddrTcp) ||
      (SD_NULL_PTR == LpIpAddrUdp) ||
      (SD_NULL_PTR == LpEntryData) ||
      (SD_NULL_PTR == LpRemoteAddr))
  {
    /* parasoft-suppress CERT_C-EXP34-a-1 "Pointer parameter validated for NULL as per ruleset. Logic skipped as per ruleset." */
    LucSkipLogic = SD_TRUE;
  }
  ...
  if (SD_FALSE == LucSkipLogic)
  {
    ...
    LpServerServiceStatic =
        &Sd_GaaServerServiceStatic[LpEvHandlerStatic->usServerServiceIndex];
#else
    LpServerServiceStatic = &(Sd_GpConfigPtr->pSd_GaaServerServiceStatic
                             [LpEvHandlerStatic->usServerServiceIndex]);
#endif
    ...
    if (Sd_GaaAclPolicyFlag == SD_TRUE)
    {
      if ((LucOptions_ACL == SD_TWO) || (LucOptions_ACL == SD_THREE))
      {
        if (SD_FALSE == (LpEvHandlerSubGrp->stEvHandlerFlag).blTcpActiveEv)
        {
          LddReturnValue = Sd_AddClientToFanoutTcp(LpServerServiceStatic,
                                                   LpEvHandler, LpEvHandlerStatic, LpEvHandlerSubGrp, LpIpAddrTcp,
                                                   LucOptionsCode, Sd_FoundTcpIndex);
        }
        else
        {
          LddReturnValue = Sd_AddClientCheck(LpEvHandlerSubGrp, (boolean)SD_TRUE,
                                             LpIpAddrTcp);
        }
      }
      ...
      if ((LucOptions_ACL == SD_ONE) || (LucOptions_ACL == SD_THREE))
      {
        if ((SD_FALSE == (LpEvHandlerSubGrp->stEvHandlerFlag).blUdpActiveEv))
        {
          LddReturnValue = Sd_AddClientToFanoutUdp(LpServerServiceStatic,
                                                   LpEvHandler, LpEvHandlerStatic, LpEvHandlerSubGrp, LpIpAddrUdp,
                                                   LucOptionsCode, Sd_FoundUdpIndex);
        }
...
if ((SD_NULL_PTR != LpEvHandler) && (SD_NULL_PTR != LpEvHandlerStatic) &&
    (SD_NULL_PTR != LpEvHandlerSubGrp))
{
  #if (STD_ON == SD_PRE_COMPILE_SINGLE)
    LpServerServiceStatic = &Sd_GaaServerServiceStatic[LpEvHandlerStatic->usServerServiceIndex];
      ...
      /* parasoft-end-suppress CERT_C-CON32-a-2 */
    {
      ...
      else
      {
        ...
        if ((uint16)LpEvHandler->ucNoOfSubGrpsUdp ==
            ((uint16)(LpEvHandlerStatic->usMulticastThreshold - (uint16)SD_ONE)))
        {
          LusBaseIndex = (uint16)LpEvHandlerStatic->usEvHandlerSelfIndex *
                        SD_SIXTEEN;
          for (LusCount = LusBaseIndex; LusCount < (uint16)(LusBaseIndex +
      ...
      /* parasoft-end-suppress CERT_C-CON32-a-2 */
        ...
        /*LogicAnalyser WRN04: Manually Verified*/
        else if ((uint16)LpEvHandler->ucNoOfSubGrpsUdp < (uint16)(LpEvHandlerStatic->usMulticastThreshold - (uint16)SD_ONE))
        {
      /* parasoft-begin-suppress CERT_C-POS49-a-2
        "Reason: Sd_State_c_Cert_REF_3" */
      /* parasoft-begin-suppress CERT_C-CON32-a-2
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_NULL_PTR != LpClientService) &&
    (SD_NULL_PTR != LpClientServiceStatic))
  {
    if (SD_NULL_PTR != LpClientServiceStatic->pTcpSoConGrp)
    {
      ...
      stClientStaticFlag is not equal to False. */
        if ((SD_TRUE == (LpClientServiceStatic->stClientStaticFlag).blActivationRoGrpIdPresent) && (SD_NO_OF_SOCKET_CONNECTIONS > LusCount))
        {
          (void)SoAd_DisableSpecificRouting(
...
/* parasoft-begin-suppress CERT_C-DCL00-b-3 
  "Reason: Sd_State_c_Cert_REF_2" */
static void Sd_ClientServiceWaitEntered(
    Sd_ClientServiceType *LpClientService,
    const Sd_ClientServiceStaticType *LpClientServiceStatic)
/* parasoft-end-suppress CERT_C-API00-a-3 */
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  {
    LusBaseIndex = (uint16)
                       LpClientServiceStatic->pTcpSoConGrp->usBaseSocketConIndex;
    LusNoOfElements = (uint16)
                          LpClientServiceStatic->pTcpSoConGrp->usNoOfSockets;
    for (LusCount = LusBaseIndex; (LusCount < (uint16)(LusBaseIndex +
                                                       (uint16)LusNoOfElements));
         LusCount++)
    {
  ...
  {
    LusBaseIndex = (uint16)
                       LpClientServiceStatic->pUdpSoConGrp->usBaseSocketConIndex;
    LusNoOfElements = (uint16)
                          LpClientServiceStatic->pUdpSoConGrp->usNoOfSockets;
    for (LusCount = LusBaseIndex; (LusCount < (uint16)(LusBaseIndex +
                                                       (uint16)LusNoOfElements));
         LusCount++)
    {
...
/* parasoft-begin-suppress CERT_C-DCL00-b-3 
  "Reason: Sd_State_c_Cert_REF_2" */
static void Sd_ClientStateDown(
    ...
    else
    {
      ...
      if (SD_NULL_PTR != LpClientServiceStatic->pTcpSoConGrp)
      {
        LusBaseIndex = (uint16)
                           LpClientServiceStatic->pTcpSoConGrp->usBaseSocketConIndex;
        LusNoOfElements = LpClientServiceStatic->pTcpSoConGrp->usNoOfSockets;
        Sd_OpenSoConsAndEnableRouting(LusBaseIndex, LusNoOfElements,
...
if ((SD_NULL_PTR != LpClientService) &&
  (SD_NULL_PTR != LpClientServiceStatic))
  {
    ...
    if ((LpClientService->enClientPhase == SD_CLIENT_REPETITION) && 
    Sd_GblBswClientRepitionStateFlag)
    {
      LpClientService->ulClientDelayCount =
          LpClientTimer->ulInitialFindRepetitionsBaseDelay;
    ...
    {
      LusBaseIndex = (uint16)LpClientServiceStatic->pTcpSoConGrp->usBaseSocketConIndex;
      LusNoOfElements = LpClientServiceStatic->pTcpSoConGrp->usNoOfSockets;
      LblSoConOnline = Sd_CheckSoConOnline(LusBaseIndex, LusNoOfElements,
                                          LpInstanceStatic);
    }
    /* polyspace +10 RTE:UNR [Justified:Low] "This condition shall be False when
    pUdpSoConGrp is not equal to SD_NULL_PTR. */
    if ((SD_FALSE == LblSoConOnline) &&
        (LpClientServiceStatic->pUdpSoConGrp != SD_NULL_PTR))
    {
      LusBaseIndex = (uint16)LpClientServiceStatic->pUdpSoConGrp->usBaseSocketConIndex;
      LusNoOfElements = LpClientServiceStatic->pUdpSoConGrp->usNoOfSockets;
      LblSoConOnline = Sd_CheckSoConOnline(LusBaseIndex, LusNoOfElements,
                                          LpInstanceStatic);
    }
    if (LblSoConOnline)
    {
      ...
      else
      {
        ...
        {
          Sd_SendFind(LpClientServiceStatic, (uint32)SD_ZERO);
          ...
          else
          {
            ...
            for (LusCount = (uint16)SD_ZERO;
                LusCount < (uint16)LpClientService->ucNoOfOffersSent; LusCount++)
            {
              LusMultiple = LusMultiple * (uint16)SD_TWO;
            }
            LpClientService->ulClientDelayCount = (uint32)LusMultiple *
              (uint32)LpClientTimer->ulInitialFindRepetitionsBaseDelay;
          }
        }
      }
    }
    else
    {
      ...
      LpClientService->enClientPhase = SD_CLIENT_INITIAL_WAIT;
      Sd_ClientServiceWaitEntered(LpClientService, LpClientServiceStatic);
      LpClientService->ulOfferTtlCount = (uint32)SD_ZERO;

  #if (STD_ON == SD_CONSUMED_EV_GRP_CONFIGURED)
      LusBaseIndex = (uint16)LpClientServiceStatic->usBaseConEventGrp;
      LusNoOfElements = (uint16)LpClientServiceStatic->usNoOfConEventGrp;

      for (LusCount = LusBaseIndex; LusCount < 
        ((uint16)(LusBaseIndex + LusNoOfElements)); LusCount++)
      {
        /*polyspace +5 RTE:IDP [Justified:Low] "Pointers with this orange flag are
              made sure that they are within their range by having necessary boundary
              checks in order to prevent them from being illegally dereferenced." */
        LpConsumedEvGrp = &Sd_GaaConsumedEvGrp[LusCount];
        LpConsumedEvGrp->ulConEvGrpTtlCount = (uint32)SD_ZERO;
      }
  #endif
    }
  }
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
    ...
    (SD_NULL_PTR != LpClientServiceStatic))
  {
    ...
    if (SD_CLIENT_SERVICE_RELEASED == LpClientService->enClientSetState)
    {
      ...
      if ((LusBaseIndex + LusNoOfElements) <= SD_NO_OF_CONSUMED_EV_GRP)
      {
            ...
            LusCount < ((uint16)(LusBaseIndex + LusNoOfElements)); LusCount++)
        {
          ...
          &(Sd_GpConfigPtr->pSd_GaaConsumedEvGrpStatic[LusCount]);
  #endif
    ...
    if (LblOfferTtlExpired)
    {
      ...
      /* polyspace +3 RTE:UNR [Justified:Low] "This condition shall be True when
        the LusBaseIndex + LusNoOfElements is  greater to SD_NO_OF_CONSUMED_EV_GRP */
      if ((LusBaseIndex + LusNoOfElements) <= (uint16)SD_NO_OF_CONSUMED_EV_GRP)
      {
        for (LusCount = LusBaseIndex;
            LusCount < ((uint16)(LusBaseIndex + LusNoOfElements)); LusCount++)
        {
...
#endif
{
  ...
  if ((SD_NULL_PTR != LpClientService) &&
    (SD_NULL_PTR != LpConsumedEvGrp) &&
    (SD_NULL_PTR != LpConsumedEvGrpStatic))
  {
    /*polyspace +4 RTE:IDP [Justified:Low] "Pointers with this orange flag are
    made sure that they are within their range by having necessary boundary
    checks in order to prevent them from being illegally dereferenced." */
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  LblMatchRecd = (boolean)SD_FALSE;
  LblWildcardMatch = (boolean)SD_FALSE;
  LusMatchCount = (uint16)SD_ZERO;
  LusWildcardMatchCount = (uint16)SD_ZERO;
  LddConnectionReturnValue = (Std_ReturnType)TCPIP_E_NOT_OK;
    ...
    (SD_NULL_PTR != LpClientServiceStatic->pTcpSoConGrp))
  {
    ...
    for (LusCount = LusBaseIndex; (LusCount < (uint16)(LusBaseIndex +
      (uint16)LusNoOfSoCon)) && (!LblMatchRecd); LusCount++)
    {
      /* polyspace +3 RTE:UNR [Justified:Low] "This condition shall be true when
    ...
    else if (LblWildcardMatch)
    {
      ...
      if (LddAssignedSoConId == Sd_GaaSoConId[LusWildcardMatchCount])
      {
        ...
        Sd_GaaSoCon[LusWildcardMatchCount].stSoConFlag.blWildcardSocketClient = SD_TRUE;
      }
    }
    /* polyspace +4 RTE:UNR [Justified:Low] "This condition shall be True when
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
 ...
 Required Rules */
  ...
  /*  Required Rule INT02 */
  /* PRQA S 4447 2 */
  /* parasoft-begin-suppress CERT_C-MSC12-k-3 "Reson: LddStIpAddr is initialized
   to zero before its use and is used in SoAd_GetRemoteAddr API which fills the
    structure with valid data or returns with failure. Hence it does not cause 
    any issue." */
  (void)SD_MEMSET(&LddStIpAddr, (sint32)SD_ZERO, sizeof(TcpIp_SockAddrType));
  /* parasoft-end-suppress CERT_C-MSC12-k-3 */
  /* polyspace +3 RTE:UNR [Justified:Low] "This condition shall be true when
    the LpClientServiceStatic is  equal to SD_NULL_PTR . */
  if ((SD_NULL_PTR != LpClientService) &&
      (SD_NULL_PTR != LpClientServiceStatic) &&
      ...
      (SD_NULL_PTR != LpClientServiceStatic->pUdpSoConGrp))
  {
    ...
    for (LusCount = LusBaseIndex; (LusCount <
        (uint16)(LusBaseIndex + (uint16)LusNoOfSoCon)) &&
      ...
      if (LpClientService->ddOfferSoConIndexUdp < SD_NO_OF_SOCKET_CONNECTIONS)
      {
                             ...
                             .stSoConFlag.blSocketOpened))
        {
          ...
          Sd_GaaSoCon[LpClientService->ddOfferSoConIndexUdp]
              .stSoConFlag.blSocketOpened = SD_TRUE;
          LddConnectionReturnValue = (TcpIp_ReturnType)TCPIP_E_OK;
        }
        else
        {
          /* SWS_SD_00761, SD_SRS_2719, ETC_FT_1248, SD_SDD_0179*/
          LddConnectionReturnValue = (TcpIp_ReturnType)
              SoAd_IsConnectionReady(Sd_GaaSoConId[LpClientService->ddOfferSoConIndexUdp],
                                     LpIpAddrUdp);
        }
...

{
      ...
      (SD_NULL_PTR != LpConsumedEvGrpStatic))
  {
      ...
      /* parasoft-end-suppress CERT_C-CON32-a-2 */
    {
      ...
      /* parasoft-begin-suppress CERT_C-CON32-a-2
        "Reason: Sd_State_c_Cert_REF_4" */
        if (SD_TRUE == (LpConsumedEvGrpStatic->
            stConsumedEvGrpStaticFlag).blTcpActivationRoGrpIdPresent)
      /* parasoft-end-suppress CERT_C-POS49-a-2 */
      /* parasoft-end-suppress CERT_C-CON32-a-2 */
        {
          if (LpClientService->ddOfferSoConIndexTcp < SD_NO_OF_SOCKET_CONNECTIONS)
      ...
      /* parasoft-begin-suppress CERT_C-CON32-a-2
        "Reason: Sd_State_c_Cert_REF_4" */
        if (SD_TRUE == (LpConsumedEvGrpStatic->
            stConsumedEvGrpStaticFlag).blUdpActivationRoGrpIdPresent)
      /* parasoft-end-suppress CERT_C-POS49-a-2 */
      /* parasoft-end-suppress CERT_C-CON32-a-2 */
        {
          if (LpClientService->ddOfferSoConIndexUdp < SD_NO_OF_SOCKET_CONNECTIONS)
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  SoAd_SoConIdType LddSoConId;
  Sd_SoConType *LpSoConStructure;
  boolean LblActiveConnFound;
  LddSoConId = SD_NO_OF_SOCKET_CONNECTIONS;
  LblActiveConnFound = (boolean)SD_FALSE;
  if (SD_NULL_PTR != LpClientService)
  {
    SchM_Enter_Sd_MODE_STATUS_PROTECTION();
    /* polyspace +3 RTE:UNR [Justified:Low] "This condition shall be true when
    ...
    if (SD_TRUE == (LpClientService->stClientFlag).blTcpActive)
    {
      ...
      LblActiveConnFound = (boolean)SD_TRUE;
    }
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  /* SWS_SD_00711 */
  {
    ...
    made sure that they are within their range by having necessary boundary
    checks in order to prevent them from being illegally dereferenced." */
    if ((SD_TRUE == LpConsumedEvGrp->stConsumedEvGrpFlag.blMulticastRoutingEnabled) &&
      ...
      /* parasoft-end-suppress CERT_C-CON32-a-2 */
    {
          ...
          (LpSoConGrp->blWildcardConEvGrp))
      {
        ...
        LddSoConIdMulticast = Sd_GaaSoConId[LpSoConGrpMulticast->usBaseSocketConIndex];
        /* polyspace +6 RTE:UNR [Justified:Low] "This condition shall
    ...
    if ((SD_TRUE ==
        ...
        ((SD_TRUE == (LpConsumedEvGrpStatic->stConsumedEvGrpStaticFlag).blUdpActivationRoGrpIdPresent)))
    {
      SchM_Enter_Sd_MODE_STATUS_PROTECTION();
      (LpConsumedEvGrp->stConsumedEvGrpFlag).blMulticastRoutingEnabled = SD_FALSE;
      (void)SoAd_DisableSpecificRouting(
          LpConsumedEvGrpStatic->ddUdpActivationRoGrpId,
...
/* polyspace-end RTE:UNR [Justified:Low] "Refer Sd_c_Poly REF_1"*/
#define SD_STOP_SEC_CODE
#include "Sd_MemMap.h"
#endif

/*******************************************************************************
** Function Name        : Sd_CheckSoConOnline                                 **
**                                                                            **
** Service ID           : NA                                                  **
**                                                                            **
** Description          : Checks if any SoCon in the given range is ONLINE.   **
**                                                                            **
** Sync/Async           : Synchronous                                         **
**                                                                            **
** Re-entrancy          : Reentrant                                           **
**                                                                            **
** Input Parameters     : LusBaseIndex - Base index in Sd_GaaSoConId array    **
**                      : LusNoOfElements - Number of elements to check       **
**                                                                            **
** InOut parameter      : None                                                **
**                                                                            **
** Output Parameters    : None                                                **
**                                                                            **
** Return parameter     : boolean - SD_TRUE if any SoCon is ONLINE, else SD_FALSE **
**                                                                            **
** Preconditions        : None                                                **
**                                                                            **
** Remarks              : Global Variable(s)  : Sd_GaaSoConId                 **
**                                                                            **
**                        Function(s) invoked : SoAd_GetSoConMode             **
**                                                                            **
...
/* parasoft-begin-suppress CERT_C-DCL00-b-3 
  "Reason: Sd_State_c_Cert_REF_2" */
static boolean Sd_CheckSoConOnline(uint16 LusBaseIndex, uint16 LusNoOfElements,
                                  const Sd_InstanceStaticType *LpInstanceStatic)
/* parasoft-end-suppress CERT_C-API00-a-3 */
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  boolean LblOnlineFound = (boolean)SD_FALSE;
  uint16 LusCount;
  SoAd_SoConIdType LddSoConId;
  SoAd_SoConModeType LenSoConMode;
  for (LusCount = LusBaseIndex; LusCount < (uint16)
      (LusBaseIndex + LusNoOfElements); LusCount++)
  {
    /*polyspace +5 RTE:OBAI [Justified:Low] "Pointers with this orange flag are
    made sure that they are within their range by having necessary boundary
    checks in order to prevent them from being out of bounds." */
    ...
    if (LenSoConMode == SOAD_SOCON_ONLINE)
    {
      LblOnlineFound = (boolean)SD_TRUE;
    }
  }
  if (!LblOnlineFound)
  {
...
/*******************************************************************************
** Function Name        : Sd_OpenSoConsAndEnableRouting                                 **
**                                                                            **
** Service ID           : NA                                                  **
**                                                                            **
** Description          : Opens SoCons and enables routing for the given range. **
**                                                                            **
** Sync/Async           : Synchronous                                         **
**                                                                            **
** Re-entrancy          : Reentrant                                           **
...
**                                                                            **
**                        Function(s) invoked : SoAd_OpenSoCon,               **
**                                              SoAd_EnableSpecificRouting    **
**                                                                            **
** Design ID            : SD_SDD_XXXX                                         **
*******************************************************************************/
...
/* parasoft-begin-suppress CERT_C-DCL00-b-3 
  "Reason: Sd_State_c_Cert_REF_2" */
static void Sd_OpenSoConsAndEnableRouting(uint16 LusBaseIndex, uint16 LusNoOfElements,
                                          const Sd_ClientServiceStaticType *LpClientServiceStatic)
/* parasoft-end-suppress CERT_C-API00-a-3 */
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if (SD_NO_OF_SOCKET_CONNECTIONS > (LusBaseIndex + LusNoOfElements))
  {
    for (LusCount = LusBaseIndex; LusCount < (uint16)(LusBaseIndex +
          LusNoOfElements); LusCount++)
    {
      ...
      /* parasoft-begin-suppress CERT_C-CON32-a-2
        "Reason: Sd_State_c_Cert_REF_4" */
      if (SD_TRUE == 
        (LpClientServiceStatic->stClientStaticFlag).blActivationRoGrpIdPresent)
      /* parasoft-end-suppress CERT_C-POS49-a-2 */
      /* parasoft-end-suppress CERT_C-CON32-a-2 */
      {
