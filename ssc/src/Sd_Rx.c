...
/* #section Sd_Rx_c_MISRA_REF_1
* Violates MISRAC2012-RULE_8_7-a-4: "Reason: This deviation is case to case 
* for global variable or function referenced only in the translation unit where 
* it is defined. Justification: Variable is accessed in other file via pointer 
* or function is accessed in other files configuration based." */

/* #section Sd_Rx_c_Cert_REF_1
...
#include "Sd_MemMap.h"
static Std_ReturnType Sd_FillTypeOfEntry(
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static Std_ReturnType Sd_CheckIndexInRange(
    uint8 LucIndex1,
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_GetOptionConfig(
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_MatchOtherServSingle(
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static boolean Sd_MatchOtherServ(
    const uint8 LpOtherServStatic[],
    uint16 LusOtherServIndex,
    uint16 LusOtherServLength,
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static Std_ReturnType Sd_SubscribeRecdActions(
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_OfferRecdInitialWait(
    Sd_ClientServiceType * LpClientService,
    const Sd_ClientServiceStaticType * LpClientServiceStatic,
    const TcpIp_SockAddrType * LpIpAddrTcp,
    const TcpIp_SockAddrType * LpIpAddrUdp,
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_OfferRecdRepetition(
    Sd_ClientServiceType * LpClientService,
    const Sd_ClientServiceStaticType * LpClientServiceStatic,
    const TcpIp_SockAddrType * LpIpAddrTcp,
    const TcpIp_SockAddrType * LpIpAddrUdp,
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static Std_ReturnType Sd_MulticastMatchSoConGroup(
    const Sd_InstanceStaticType * LpInstanceStatic,
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static Std_ReturnType Sd_MulticastMatchSocket(
    const Sd_InstanceStaticType * LpInstanceStatic,
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_SubscribeAckRecdActions(
    const Sd_InstanceType * LpInstance,
    const Sd_InstanceStaticType * LpInstanceStatic,
    const Sd_ClientServiceType * LpClientService,
    Sd_ConsumedEvGrpType * LpConsumedEvGrp,
...
#define SD_START_SEC_CODE
#include "Sd_MemMap.h"
static void Sd_SubscribeNackRecdActions(
    const Sd_ClientServiceStaticType * LpClientServiceStatic,
    const TcpIp_SockAddrType * LpRemoteAddr,
...
"Reason: Sd_Rx_c_Cert_REF_1" */
/* parasoft-begin-suppress CERT_C-DCL00-b-3 
  "Reason: Sd_Rx_c_Cert_REF_2" */
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_NULL_PTR != LpSduDataPtr) && (SD_NULL_PTR != LpSessionId) && 
  (SD_NULL_PTR != LpRebootFlag))
  {
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if (SD_NULL_PTR == LpEntry)
  {
    LddReturnValue = E_NOT_OK;
  }
  else if (LulTtlSec > (uint32)SD_ZERO)
  {
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((uint8)(LucNoOfOptions1 + LucIndex1) > LucTotalNoOfOptions)
  {
    LddReturnValue = E_NOT_OK;
  }
  /*LogicAnalyser WRN04: Manually Verified*/
  else if ((uint8)(LucNoOfOptions2 + LucIndex2) > LucTotalNoOfOptions)
  {
    LddReturnValue = E_NOT_OK;
  }
  else
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  uint32 LulTtlSec;
  Std_ReturnType LddReturnValue;
  boolean LblEntryType1 = (boolean)SD_FALSE;
  uint8 LucData;

  if ((SD_NULL_PTR != LpPduDataPtr) && (SD_NULL_PTR != LpEntry))
  {
    ...
    { /*SWS_SD_00624
      ...
      else
      {
        ...
        (LpEntry->unEntryData).stEntry2.ucNoOfOptions1 =
           (uint8) (LucData & (uint8)SD_HIGH_NIBBLE_MASK) >> (uint8)SD_FOUR;
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_NULL_PTR != LpOptionsPtr) &&
      (SD_NULL_PTR != LpOptPtrIndex) &&
      (SD_NULL_PTR != LpRxOptionsData) &&
      (SD_NULL_PTR != LpNoOfOptions) &&
      (SD_NULL_PTR != LpMalformedOption))
  {
    LusOtherServIndex = SD_ZERO;

    /* parasoft-begin-suppress CERT_C-MSC41-a-1 "Reason: This deviation  
    * is case to case for the The string literal is embedded directly in the code." 
    * "Justification: The string literal is embedded directly in the code 
    * as it is necessary for the functionality." */
    LddMatchReturn = Sd_MatchString(&LpOptionsPtr[*LpOptPtrIndex],
                                    (const uint8 *) SD_OTHERSERV_STRING,
                                    &LusOtherServIndex);
    ...
    else
    {
      ...
      {
        LucNoOfChar = LucNoOfChar - (uint8) SD_TEN;

        while ((LusCount < (uint16) LucNoOfChar) &&
               (LusCount < (uint16) (SD_OTHERSERV_SIZE - SD_ONE)))
        {
          ...
          * ensure the correct type is used." */
          ...
          (LpRxOptionsData[*LpNoOfOptions].unOptionsData).aaOtherServ[LusCount] =
            (uint8)(LpOptionsPtr[((uint16)*LpOptPtrIndex + LusOtherServIndex + 
            (uint16) SD_TEN + LusCount)]);
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_NULL_PTR != LpInstanceStatic) && (SD_NULL_PTR != LpOptionsPtr) 
  && (SD_NULL_PTR != LpRxOptionsData) && (SD_NULL_PTR != LpNoOfOptions))
  {
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_NULL_PTR != LpInstanceStatic) && (SD_NULL_PTR != LpRxOptionsData)
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_NULL_PTR != LpCurrOption) &&
      (SD_NULL_PTR != LpOtherServStatic) &&
      (SD_NULL_PTR != LpOtherServMatch))
  {
    /* SWS_SD_00293c
       SWS_SD_00293d */
    /*polyspace +4 RTE:IDP [Justified:Low] "Pointers with this orange flag are
    ...
    if (SD_CONFIGURATION_OPTION == LpCurrOption->enTypeOfOption)
    {
      if (!*LpOtherServMatch)
      {
        ...
        *LpOtherServMatch = (boolean)SD_TRUE;
        for (LucCount2 = SD_ZERO; ((uint16) LucCount2 < LusOtherServLength) &&
          (!LblBreak); LucCount2++)
        {
          ...
          if (LpCurrOption->unOptionsData.aaOtherServ[LusDataIndex] !=
              LpOtherServStatic[LusOtherServIndex + (uint16) LusDataIndex])
          {
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_NULL_PTR == LpOtherServStatic) ||
      (SD_NULL_PTR == LpEntry) ||
      (SD_NULL_PTR == LpOptionsData))
  {
    LblOtherServMatch = (boolean)SD_FALSE;
  }
  else if (SD_ZERO == LusOtherServLength)
  {
    LblOtherServMatch = (boolean)SD_FALSE;
  }
  else
  {
    ...
    for (LucCount = LucIndex1; LucCount < (uint8) (LucIndex1 + LucNoOfOptions1);
         LucCount++)
    {
    ...
    for (LucCount = LucIndex2; LucCount < (uint8) (LucIndex2 + LucNoOfOptions2);
         LucCount++)
    {
      LpCurrOption = &LpOptionsData[LucCount];
      Sd_MatchOtherServSingle(LpCurrOption, LpOtherServStatic,
...
/* parasoft-end-suppress MISRAC2012-RULE_8_7-a-4 */
{
      ...
      (SD_NULL_PTR != LpOptionsData))
  {
        ...
        (SD_SERVICE_ID_OTHERSERV == LusServiceIdStatic))
    {
    	...
    	{
    		LblServiceMatch =
    		Sd_MatchOtherServ(LpOtherServStatic, LusOtherServIndex,
      ...
      if (LblServiceMatch)
      {
        if (LusInstanceId == LpServerServiceStatic->usInstanceId)
        {
          if (LucMajorVersion == LpServerServiceStatic->ucMajorVersion)
          {
            ...
            for (LusCount2 = LusBaseEventIndex; LusCount2 < (uint16) (LusBaseEventIndex +
              (uint16) LpServerServiceStatic->usNoOfEvHandlers);
                 LusCount2++)
            {
#if (STD_ON == SD_PRE_COMPILE_SINGLE)
...
/* parasoft-end-suppress MISRAC2012-RULE_8_7-a-4 */
{
  ...
  /* Null pointer check for all pointer parameters */
  if ((SD_NULL_PTR != LpInstanceStatic) && (SD_NULL_PTR != LpClientServiceStatic))
  {
    /* polyspace +6 RTE:UNR [Justified:Low] "This condition shall be True when
      ...
      (SD_NULL_PTR != LpIpAddrUdp))
  {
    ...
    if (SD_TTL_SEC_MAX == LulTtlSec)
    {
      /* parasoft-begin-suppress CERT_C-POS49-a-2
        "Reason: Sd_Rx_c_Cert_REF_3" */
      /* parasoft-begin-suppress CERT_C-CON32-a-2
        "Reason: Sd_Rx_c_Cert_REF_4" */
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
      ...
      (SD_NULL_PTR != LpRemoteAddr))
  {
    ...
    if (E_OK == LddReturnValue)
    {
      ...
      for (LusCount = LusBaseIndex; LusCount < (uint16) (LusBaseIndex +
          (uint16) LpClientServiceStatic->usNoOfConEventGrp);
           LusCount++)
      {
#if (STD_ON == SD_PRE_COMPILE_SINGLE)
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
      ...
      (SD_NULL_PTR != LpSoConGrpIndex))
  {
    ...
    for (LusCount = LusBaseIndex; (LusCount < (uint16) (LusBaseIndex +
      (uint16) LpConsumedEvGrpStatic->ddNoOfMulticastSoConGrp)) &&
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_NULL_PTR != LpInstanceStatic) &&
      ...
      (SD_NULL_PTR != LpSoConGrpStatic) &&
      (SD_NULL_PTR != LpSoConId))
  {
...
#if (SD_SET_REM_ADDR_OF_CLIENT_RX_MULTICAST_SO_CON == STD_ON)
    for (LusCount = LusBaseIndex; (LusCount < (uint16) (LusBaseIndex +
        (uint16) LpSoConGrpStatic->usNoOfSockets)) &&
        (!LblMatchRecd); LusCount++)
    {
      /* polyspace +10 RTE:UNR [Justified:Low] "This condition shall be False when
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_NULL_PTR != LpInstanceStatic) && (SD_NULL_PTR != LpEntryData) && 
  (SD_NULL_PTR != LpOptionsData) && (SD_NULL_PTR != LpInstance))
  {
    LpConsumedEvGrp = &Sd_GaaConsumedEvGrp[SD_ZERO];
  #if (STD_ON == SD_PRE_COMPILE_SINGLE)
    LpConsumedEvGrpStatic = &Sd_GaaConsumedEvGrpStatic[SD_ZERO];
  ...
  #endif
    LpClientService = &Sd_GaaClientService[SD_ZERO];
        ...
        (!LblMatchRecd); LusCount++)
    {
          ...
          (LucMajorVersion == LpClientServiceStatic->ucMajorVersion))
      {
        ...
        for (LusCount2 = LusBaseIndex2; (LusCount2 < (uint16) (LusBaseIndex2 +
            (uint16) LpClientServiceStatic->usNoOfConEventGrp)) &&
            (!LblMatchRecd); LusCount2++)
        {
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  if ((SD_NULL_PTR != LpEntryData) && (SD_NULL_PTR != LpOptionsData) && 
  (SD_NULL_PTR != LpInstanceStatic))
  {
    /*polyspace +4 RTE:NIV [Justified:Low] "Pointers with this orange flag are
          made sure that they are initialized with proper Value " */
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  uint16 LaaBaseIndex[SD_TWO];
  uint16 LaaNoOfElements[SD_TWO];
  ...
  boolean LblMatchRecd;
  uint8 LucTotalSoConGrp;
  uint8 LucMainCount;

  if ((SD_NULL_PTR != LpClientServiceStatic) && 
  (SD_NULL_PTR != LpBaseEntriesSduPtr) &&
  (SD_NULL_PTR != LpRemoteAddr))
  {
  #if (STD_ON == SD_PRE_COMPILE_SINGLE)
    LpInstanceStatic =
    ...
    if (LblSubscribeAckRecd)
    {
      ...
      if (SD_NULL_PTR != LpClientServiceStatic->pTcpSoConGrp)
      {
        LaaBaseIndex[LucTotalSoConGrp] = LpClientServiceStatic->pTcpSoConGrp->usBaseSocketConIndex;
        LaaNoOfElements[LucTotalSoConGrp] = LpClientServiceStatic->pTcpSoConGrp->usNoOfSockets;
        ++LucTotalSoConGrp;
        LddReturnValue = E_OK;
      }
      ...
      if (SD_NULL_PTR != LpClientServiceStatic->pUdpSoConGrp)
      {
        LaaBaseIndex[LucTotalSoConGrp] = LpClientServiceStatic->pUdpSoConGrp->usBaseSocketConIndex;
        LaaNoOfElements[LucTotalSoConGrp] = LpClientServiceStatic->pUdpSoConGrp->usNoOfSockets;
        ++LucTotalSoConGrp;
        LddReturnValue = E_OK;
      }
      ...
      if (E_OK == LddReturnValue)
      {
        for (LucMainCount = (uint8) SD_ZERO; LucMainCount < LucTotalSoConGrp; LucMainCount++)
        {
          for (LusCount = LaaBaseIndex[LucMainCount]; LusCount < (uint16) (LaaBaseIndex[LucMainCount] + 
            LaaNoOfElements[LucMainCount]); LusCount++)
          {
            /*polyspace +10 RTE:OBAI [Justified:Low] "Pointers with this orange flag are
            made sure that they are within their range by having necessary boundary
            checks in order to prevent them from being out of bounds." */
...
/* parasoft-end-suppress CERT_C-DCL00-b-3 */
{
  ...
  #if (STD_ON == SD_PRE_COMPILE_SINGLE)
    LpClientServiceStatic = &Sd_GaaClientServiceStatic[SD_ZERO];
  #else
    LpClientServiceStatic =
        &(Sd_GpConfigPtr->pSd_GaaClientServiceStatic
          [Sd_GpConfigPtr->pSd_GaaClientHandleMapArray[SD_ZERO]]);
  #endif
